"""Anthropic-compatible native Messages transport implementation."""
...
from providers.anthropic_messages_http import (
    iter_grouped_sse_events,
    iter_sse_line_chunks,
    read_streaming_error_body_preview,
)
    ...
    async def _raise_for_status(
        ...
        try:
            ...
            if self._config.log_api_error_tracebacks:
                preview, truncated = await read_streaming_error_body_preview(
    ...
    async def _iter_stream_chunks(
        ...
        *,
        ...
        if self.stream_chunk_mode == "line" and isinstance(
            ...
            async for event in iter_grouped_sse_events(response):
        ...
        if self.stream_chunk_mode == "line":
            async for chunk in iter_sse_line_chunks(response):
                yield chunk
            return
